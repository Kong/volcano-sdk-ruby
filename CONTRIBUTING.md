...
VOLCANO_SDK_CONTRACT_FIXTURE="$PWD/tests/fixtures/sdk-contract-dry-run.json" \
  bundle exec cucumber features/contract --dry-run --strict --format progress
gem build volcano-sdk.gemspec
bash .github/scripts/smoke-gem.sh volcano-sdk-*.gem
```

Regenerate a changed wire snapshot with `bin/generate-openapi`. The dry run
checks phrase bindings without creating fixtures or exercising live behavior.
