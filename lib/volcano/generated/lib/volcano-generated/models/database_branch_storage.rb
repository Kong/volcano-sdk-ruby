...
module Volcano::Generated
  ...
  class DatabaseBranchStorage < ApiModelBase
    attr_accessor :id

    # Branch name
    attr_accessor :name

    # Bytes this branch has diverged from its parent. Pages the branch still shares with the parent are not counted, so this is what the branch actually adds to the total rather than its apparent size. 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Volcano::Generated::DatabaseBranchStorage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Volcano::Generated::DatabaseBranchStorage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @storage_bytes < 0
      true
    end
