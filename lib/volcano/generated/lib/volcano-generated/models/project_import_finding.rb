...
module Volcano::Generated
  class ProjectImportFinding < ApiModelBase
    attr_accessor :code
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Volcano::Generated::ProjectImportFinding` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Volcano::Generated::ProjectImportFinding`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^(vercel|volcano|import)\.[a-z0-9_]+$/)
      if @code !~ pattern
    ...
    def valid?
      ...
      return false if @code !~ Regexp.new(/^(vercel|volcano|import)\.[a-z0-9_]+$/)
    ...
    # Custom attribute writer method with validation
    # @param [Object] remediation Value to be assigned
    def remediation=(remediation)
      if remediation.nil?
        fail ArgumentError, 'remediation cannot be nil'
      end
    ...
    def hash
      [code, resource, disposition, impact, message, remediation].hash
    end
