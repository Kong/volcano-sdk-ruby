...
module Volcano::Generated
  class SandboxTemplate < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Volcano::Generated::SandboxTemplate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Volcano::Generated::SandboxTemplate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["ready", "unavailable", "deleting"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["ready", "unavailable", "deleting"])
    ...
    def hash
      [id, project_id, name, preset, memory_mb, status, created_at].hash
    end
