...
module Volcano::Generated
  # Service role key for admin operations. **WARNING:** Bypasses all RLS - backend use only! 
  class ServiceKey < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Volcano::Generated::ServiceKey` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Volcano::Generated::ServiceKey`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @key_prefix.to_s.length > 12
        invalid_properties.push('invalid value for "key_prefix", the character length must be smaller than or equal to 12.')
      end
    ...
    def valid?
      ...
      return false if @key_prefix.to_s.length > 12
    ...
    def key_prefix=(key_prefix)
      ...
      if key_prefix.to_s.length > 12
        fail ArgumentError, 'invalid value for "key_prefix", the character length must be smaller than or equal to 12.'
      end
    ...
    def hash
      [id, project_id, name, key_value, key_prefix, permissions, created_at, updated_at].hash
    end
