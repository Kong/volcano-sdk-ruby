...
RSpec.describe Volcano::DurableExecution do
  it 'checks and runs the public durable record consumer' do
    fixture = File.expand_path('../../tests/types/durable_records.rb', __dir__)
  ...
  it 'rejects invalid durable record consumers' do
    fixture = File.expand_path('../../tests/types_invalid/durable_records.rb', __dir__)
    output, errors, status = SteepConsumer.check(File.read(fixture))
  ...
  it 'ships durable record signatures in the gem manifest' do
    files = Gem::Specification.load(File.expand_path('../../volcano-sdk.gemspec', __dir__)).files

    expect(files).to include('sig/durable_records.rbs')
  end
end
